...
#include "device_io.h"     /* Petit FatFs configurations and declarations */
...
)
{
   ...
   do {
      ...
      if (!(dir[DIR_Attr] & AM_VOL) &&
          !mem_cmp((void*)dir, (void*)dj->fn, 11)) /* Is it a valid entry? */
               break;
...
)
{
   ...
   if (disk_readp(buf, bsect, 13, 36)) return FR_DISK_ERR;
...
//    dr = disk_readp(!buff ? 0 : rbuff, fs->dsect, (WORD)(fs->fptr % 512), rcnt);
      dr = disk_readp(rbuff, fs->dsect, (WORD)(fs->fptr % 512), rcnt); // save a few instructions
      if (dr) goto fr_abort;
...
)
{
   ...
   *bw = 0;
   ...
   while (btw) {                          /* Repeat until all data transferred */
      ...
      if (disk_writep((BYTE*)p, wcnt)) goto fw_abort; /* Send data to the sector */
